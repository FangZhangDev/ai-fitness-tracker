...
| 动作 | 组 × 次 | RIR | 休息(秒) | 要点 | 器材 |
| --- | --- | --- | --- | --- | --- |
| 上斜推胸训练器 | 3 × 8-10 | 2 | 120 | 上胸优先，肩胛后收下沉 | 上斜卧推训练器 |
| 绳索侧平举 | 3 × 12-20 | 1-2 | 60 | 小重量，手肘带动 | 龙门架 + D型手柄 |

**周四：上肢 B，背宽 + 胸 + 后束**

| 动作 | 组 × 次 | RIR | 休息(秒) | 要点 | 器材 |
| --- | --- | --- | --- | --- | --- |
| 高位下拉 | 3 × 6-10 | 2 | 120 | 想象用肘拉，不是手拉 | 高位下拉训练器 |
\`\`\`

要求：每个训练日一个「**周X：主题**」标题加一张表；
表格必须是「动作 / 组 × 次 / RIR / 休息(秒) / 要点 / 器材」这六列；
休息一列只写秒数（如 120），不要写"2-3分钟"——手表的组间倒计时直接读它；
只写我实际要练的那几天。`;
  ...
  if (!d.plans.length) {
    ...
    for (const p of d.plans) {
      ...
      for (const day of p.days) {
        ...
        parts.push("");
        parts.push("| 动作 | 组 × 次 | RIR | 休息(秒) | 要点 | 器材 |");
        parts.push("| --- | --- | --- | --- | --- | --- |");
        for (const e of day.exercises) {
          parts.push(
            `| ${cell(e.exercise)} | ${setsReps(e)} | ${range(e.rir_min, e.rir_max)} | ${cell(e.rest_sec)} | ${cell(e.cues)} | ${cell(e.equipment)} |`,
          );
        }
      }
      parts.push("");
